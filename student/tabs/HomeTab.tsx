...
import { ClinicalRecord, SickNote, Appointment, ReferralLetter } from '../../types';
...
const HomeTab: React.FC<HomeTabProps> = ({ user }) => {
  ...
  useEffect(() => {
    ...
    if (studentId) {
      ...
      const qMine = query(collection(db, "appointments"), where("studentId", "==", studentId));
      const unsubMine = onSnapshot(qMine, (snap) => {
        setMyAppointments(snap.docs.map(d => ({ id: d.id, ...d.data() } as Appointment)));
      });

      const qReferrals = query(collection(db, "referrals"), where("studentId", "==", studentId));
      const unsubReferrals = onSnapshot(qReferrals, (snap) => {
        setReferrals(snap.docs.map(d => ({ id: d.id, ...d.data() } as ReferralLetter)).sort((a, b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0)));
      });

      return () => { unsubRec(); unsubSick(); unsubQueue(); unsubMine(); unsubReferrals(); };
    }
  }, [user]);
  ...
  const timeLeftLabel = (apt: Appointment) => {
    const diff = getAppointmentDate(apt).getTime() - currentTime.getTime();
  ...
  const visibleBookings = useMemo(() => (
    myAppointments
      .filter(a => ['pending', 'approved', 'confirmed', 'doctor_unavailable'].includes(a.status))
  ...
  const userQueueInfo = useMemo(() => {
    ...
    const pendingQueue = activeQueue.filter(a => a.status === 'pending' || a.status === 'approved');
    ...
    <div className="space-y-4 lg:space-y-6 pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="space-y-4">
                 {referrals.map(referral => (
                   <button
                    ...
                    className="w-full flex items-center justify-between p-4 bg-yellow-50 border border-mPolyYellow/40 group hover:border-mPolyBlue transition-all"
                   >
                     <div className="flex items-center gap-3">
                        <i className="fa-solid fa-file-medical text-mPolyBlue"></i>
                        <div className="text-left">
                           <p className="text-[10px] font-black text-mPolyBlue uppercase">Referral Letter</p>
                           <p className="text-[8px] font-bold text-neutral-400 uppercase">{referral.recommendedSpecialist}</p>
                        </div>
                     </div>
                     <i className="fa-solid fa-download text-neutral-300 group-hover:text-mPolyBlue"></i>
                   </button>
                 ))}
        ...
        <div className="lg:col-span-2 space-y-6 animate-slide-right">
           <Card title="Booking Tracker" className="border-t-8 border-mPolyYellow">
             <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead className="text-[9px] font-black uppercase tracking-widest text-neutral-400 border-b">
                  <tr>
                    <th className="p-3">Status</th>
                    <th className="p-3">Real Date</th>
                ...
                <tbody className="divide-y text-[10px] font-bold uppercase">
                  {visibleBookings.map(apt => (
                    <tr key={apt.id}>
                      <td className="p-3 text-mPolyBlue">{apt.status}</td>
                      <td className="p-3">{new Date(apt.date).toLocaleDateString('en-GB')} {apt.time}</td>
                      <td className="p-3 text-mPolyGreen">{timeLeftLabel(apt)}</td>
                      <td className="p-3">{apt.doctorName || 'Awaiting admin'}</td>
                      <td className="p-3 normal-case max-w-xs truncate">{apt.symptoms || apt.reason}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              {visibleBookings.length === 0 && <p className="text-[10px] font-bold text-neutral-300 uppercase italic py-4">No pending or approved bookings.</p>}
             </div>
           </Card>
