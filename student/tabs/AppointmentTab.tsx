...
import { collection, addDoc, query, where, onSnapshot, serverTimestamp, getDocs, updateDoc, doc } from "https://www.gstatic.com/firebasejs/10.8.0/firebase-firestore.js";
...
const AppointmentTab: React.FC<AppointmentTabProps> = ({ user }) => {
  ...
  useEffect(() => {
    ...
    const fetchOccupied = async () => {
      ...
      const occupied = snap.docs
        .map(d => d.data())
        .filter((apt: any) => apt.status !== 'cancelled' && apt.status !== 'completed' && apt.status !== 'doctor_unavailable')
  ...
  useEffect(() => {
    ...
    const fetchOccupied = async () => {
      const q = query(collection(db, "appointments"), where("date", "==", rescheduleDate));
      const snap = await getDocs(q);
      const occupied = snap.docs
        .filter(d => d.id !== rescheduleApt.id)
        .map(d => d.data())
        .filter((apt: any) => apt.status !== 'cancelled' && apt.status !== 'completed' && apt.status !== 'doctor_unavailable')
        .map((apt: any) => apt.time);
      setRescheduleOccupiedSlots(occupied);
    };
    fetchOccupied();
  ...
  const openReschedule = (apt: Appointment) => {
    ...
    setRescheduleTime(apt.time || null);
  };

  const handleReschedule = async () => {
    if (!rescheduleApt?.id || !rescheduleTime) return;
    ...
    try {
      ...
      alert("Pending booking rescheduled.");
    } catch (error) {
      console.error(error);
  ...
  const handleBooking = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const appointmentData: Record<string, any> = {
        ...
        reason: selectedConcern || 'General Illness',
        time: selectedTime,
        date: selectedDate,
      ...
      if (user.whatsapp) appointmentData.studentWhatsapp = user.whatsapp;
  ...
  if (isBooked) {
    return (
      <div className="flex flex-col items-center justify-center py-20 animate-fade-in text-center">
        ...
        <p className="text-xs font-bold text-neutral-400 uppercase tracking-widest max-w-md mb-10">Your booking request for {selectedDate} at {selectedTime} was sent to admin. A doctor will be assigned after approval.</p>
    ...
    <div className="space-y-6 lg:space-y-8 pb-10">
              ...
              <div className="space-y-6">
                {myAppointments.length > 0 ? (
                  myAppointments.sort((a,b) => b.date.localeCompare(a.date)).slice(0, 3).map(apt => (
                    <div key={apt.id} className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 p-4 bg-slate-50 border border-neutral-100">
                      ...
                      <div className="text-right">
                        ...
                        {apt.status === 'pending' && (
                          <button
                            ...
                            className="block mt-2 bg-white border-2 border-mPolyBlue/20 text-mPolyBlue px-4 py-2 text-[8px] font-black uppercase tracking-widest hover:border-mPolyBlue"
                          >
                            Reschedule
                          </button>
                        )}
             ...
             <Card title={`Schedule for ${selectedConcern}`}>
                <div className="space-y-10">
                   <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase text-neutral-400">Doctor Assignment</label>
                      <div className="w-full bg-mPolyBlue/5 border-2 border-mPolyBlue/10 p-4 text-xs font-bold text-mPolyBlue uppercase">
                        Admin will assign the available doctor after reviewing your request.
                      </div>
                   </div>
                   ...
                   <div className="flex gap-4 pt-6 border-t border-neutral-100">
                      ...
                      <Button variant="primary" fullWidth disabled={!selectedTime} onClick={() => setStep(3)}>Continue</Button>
                   </div>
                </div>
             </Card>
           )}
                ...
                <form onSubmit={handleBooking} className="space-y-8">
                   ...
                   <div className="bg-mPolyBlue/5 p-6 border-l-8 border-mPolyBlue space-y-2">
                      ...
                      <div className="flex justify-between text-xs font-bold uppercase">
                         ...
                         <span className="text-mPolyBlue">Assigned by admin</span>
                      </div>
                   </div>
                   <div className="flex gap-4">
      ...
      {rescheduleApt && (
        <div className="fixed inset-0 bg-mPolyBlue/90 z-[600] flex items-center justify-center p-4">
          <div className="bg-white max-w-2xl w-full p-8 shadow-2xl border-t-8 border-mPolyYellow animate-slide-right">
            <div className="flex justify-between items-start gap-6 mb-8">
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-mPolyGreen">Reschedule Pending Booking</p>
                <h3 className="text-2xl font-black uppercase text-mPolyBlue">{rescheduleApt.reason}</h3>
                <p className="text-xs font-bold text-neutral-400 uppercase mt-1">Current: {rescheduleApt.date} @ {rescheduleApt.time}</p>
              </div>
              <button onClick={() => setRescheduleApt(null)} className="text-neutral-300 hover:text-red-600">
            ...
            <div className="space-y-8">
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-neutral-400">New Visit Date</label>
                <input
                  type="date"
                  min={new Date().toISOString().split('T')[0]}
              ...
              <div className="space-y-4">
                <p className="text-[10px] font-black uppercase text-neutral-400">New Available Slot</p>
                <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
                  {allSlots.map(time => {
                    const isOccupied = rescheduleOccupiedSlots.includes(time);
                    return (
                      <button
                        key={time}
                        type="button"
                        ...
                        className={`py-4 text-[10px] font-black uppercase border-2 transition-all
                          ${isOccupied ? 'bg-neutral-50 text-neutral-300 border-neutral-50 cursor-not-allowed line-through' :
                            rescheduleTime === time ? 'bg-mPolyBlue text-white border-mPolyBlue' : 'bg-white text-neutral-600 border-neutral-100 hover:border-mPolyGreen'}`}
                      >
                        {time}
                        {isOccupied && <span className="block text-[7px] text-red-400 mt-1">Booked</span>}
                      </button>
              ...
              <div className="flex gap-4 pt-6 border-t border-neutral-100">
                <Button variant="outline" fullWidth onClick={() => setRescheduleApt(null)}>Cancel</Button>
                <Button variant="primary" fullWidth disabled={!rescheduleTime || isSubmitting} onClick={handleReschedule}>
