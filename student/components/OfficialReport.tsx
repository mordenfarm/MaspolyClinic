...
const OfficialReport: React.FC<OfficialReportProps> = ({ record, student, onClose }) => {
  ...
  const verificationUrl = useMemo(() => buildVerificationUrl('clinicalRecord', record.id), [record.id]);
  ...
  useEffect(() => {
    ...
    QRCode.toDataURL(verificationUrl, {
      ...
      console.error('Failed to generate clinical record QR:', error);
    });
  }, [record.id, verificationUrl]);
    ...
    <div className="fixed inset-0 bg-white z-[1000] overflow-y-auto no-scrollbar print:static print:h-auto">
      ...
      <div id="report-content" className="max-w-4xl mx-auto p-8 lg:p-16 print:p-0 print:shadow-none print:border-none">
        ...
        <div className="grid grid-cols-2 gap-20 pt-12 border-t-2 border-neutral-100">
           ...
           <div className="text-right">
              ...
              {qrDataUrl ? (
                <img 
                  src={qrDataUrl}
                  className="w-20 ml-auto border-2 p-1 border-neutral-100 bg-white"
                  alt="Clinical record verification QR"
                />
              ) : (
                <div className="w-20 h-20 ml-auto border-2 p-1 border-neutral-100 bg-slate-50 flex items-center justify-center text-[7px] font-black uppercase text-neutral-400">QR loading</div>
              )}
           </div>
        </div>
        ...
        <footer className="mt-32 pt-8 border-t border-slate-100 flex justify-between items-center text-[10px] font-bold text-neutral-200 uppercase tracking-widest">
           ...
           <span className="break-all text-right">{verificationUrl}</span>
        </footer>
      </div>
    </div>
  );
