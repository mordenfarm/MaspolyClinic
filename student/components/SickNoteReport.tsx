...
const SickNoteReport: React.FC<SickNoteReportProps> = ({ note, onClose }) => {
  ...
  const verificationUrl = useMemo(() => buildVerificationUrl('sickNote', note.id), [note.id]);
  ...
  useEffect(() => {
    ...
    QRCode.toDataURL(verificationUrl, {
      ...
      console.error('Failed to generate medical excuse QR:', error);
    });
  }, [note.id, verificationUrl]);
        ...
        <div className="border-[12px] border-mPolyBlue p-10 lg:p-16 relative">
          ...
          <div className="relative z-10">
            ...
            <div className="mt-24 grid grid-cols-2 gap-20">
               ...
               <div className="text-right">
                  ...
                  {qrDataUrl ? (
                    <img 
                      src={qrDataUrl}
                      className="w-20 ml-auto border p-1 bg-white" 
                      alt="Medical excuse verification QR"
                    />
                  ) : (
                    <div className="w-20 h-20 ml-auto border p-1 bg-slate-50 flex items-center justify-center text-[7px] font-black uppercase text-neutral-400">QR loading</div>
                  )}
               </div>
            </div>
            ...
            <footer className="mt-20 pt-8 border-t border-slate-100 flex justify-between items-center">
               ...
               <p className="text-[9px] font-black uppercase tracking-widest text-neutral-300 break-all">{verificationUrl}</p>
            </footer>
          </div>
        </div>
      </div>
